...
def index():
    ...
    categorical_Images = []
    #Chart1
    def GenerateFigure(column):
        plt.figure()
        g = sns.countplot(x=column, hue="CHURN",palette=sns.color_palette(flatui),data=all_data)
        g.figure.set_size_inches(10,10)
        sns.despine()
        g.axes.set_title('Visulizing customer churn by '+column,fontsize=30,color="#00008B",alpha=0.6)
        g.set_xlabel(column,size = 20,color="black",alpha=0.5)
        g.set_ylabel("totalcount",size = 20,color="black",alpha=0.5)
        g.tick_params(labelsize=20,labelcolor="black")
        barImageUrl = basePath + column+"_barchart'.jpg"
        plt.savefig(barImageUrl)
        #DataObject[column+"_barImageUrl"]=barImageUrl + q
        categorical_Images.append(barImageUrl + q)
        return

    GenerateFigure('GENDER')
    GenerateFigure('PHONESERVICE')
    GenerateFigure('MULTIPLELINES')
    GenerateFigure('CONTRACT')
    GenerateFigure('PARTNER')
    DataObject["categorical_Images"] =categorical_Images
